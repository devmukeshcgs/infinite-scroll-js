...
images.forEach((image, idx) => {
  image.style.backgroundImage = `url(./src/images/${idx + 1}.jpeg)`;
});
...
if (!isMobile) {
  imgSections.forEach((section) => {
    ...
    cloneSection.classList.add("cloned");
    content.appendChild(cloneSection);
  });
}
...
document.addEventListener("click", () => (reverce = !reverce));
